...
$(document).ready(function(){
	...
	$(window).scroll(function(){
		...
		if(pos >= 200 && !isCompleteReco) {
			isCompleteReco = true;
			getProducts(sectionRecommend,'/mumarket/main/recommend');
			
		}else if(pos >= 400 && !isCompleteNew) {
			isCompleteNew = true;
			getProducts(sectionNew,'/mumarket/main/newItems');
		}else if(pos >= 600 && !isCompleteDisc) {
			isCompleteDisc = true;
			getProducts(sectionDiscount,'/mumarket/main/discountItems');
		}
		...
		$.ajax({
			...
			success: function(data){
				...
				$.each(data, function(i, item){
					...
					var imgUrl = '/mumarket/thumb/'+item.cate1+"/"+item.cate2+"/"+item.thumb1;
